...
export class POHClient {
  ...
  async registerKeyPair(keyPair: KeyPair, rotationProof?: string): Promise<RegisterKeyResult> {
    ...
    return this.registerSigningKey(keyPair.address, keyPair.signingPublicKey, proof, rotationProof)
  }
  ...
  async transfer(
    ...
    const tx     = await buildTransfer(from, to, amountPOH, nextNonce, fee, memo)
