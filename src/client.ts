...
export class POHClient {
  ...
  private readonly _localBaseUrl:  string | undefined
  ...
  constructor(options: POHClientOptions) {
    ...
    this._localBaseUrl = options.localBaseUrl?.replace(/\/$/, '')
  ...
  private _needsLocalNode(method: string, path: string): boolean {
  ...
  private async _resolveBaseUrl(method: string, path: string): Promise<string> {
    if (!this._needsLocalNode(method, path)) return this._getBaseUrl()
  ...
  private async request<T>(
    ...
    const baseUrl = await this._resolveBaseUrl(method, path)
  ...
  async submitJob(question: string, options: AskOptions = {}): Promise<AskJobRef> {
    ...
    if (budgetRaw > 0) {
      ...
      paymentTx = await signJobPayment(
        { jobId, requesterAddress, minerAddress, amount: budgetRaw, nonce, currency: feeCurrency },
  ...
  async runCompute(prompt: string, options: ComputeOptions): Promise<AskJobRef> {
    ...
    const paymentTx = await signJobPayment(
      { jobId, requesterAddress: walletAddress, minerAddress, amount, nonce, currency: feeCurrency },
      privateKeyPem,
    )
  ...
  async listSkills(): Promise<Skill[]> {
    const data = await this.request<Skill[] | { skills: Skill[] }>('GET', '/api/skills')
  ...
  async getAssets(): Promise<AssetsResult> {
    return this.request<AssetsResult>('GET', '/api/assets')
  }
   ...
   *
   * The address must equal `deriveAddressFromSigningKey(signingPublicKey)`.
   * The `proof` is a signature of the wallet address — use `createSigningProof()`.
   * Pass `rotationProof` when replacing an existing registered key.
   *
   * @example
  ...
  async registerSigningKey(
    ...
    const body: Record<string, string> = { address, signingPublicKey, proof }
    if (rotationProof) body.rotationProof = rotationProof
    // Publish the wallet's X25519 encryption key so miners can seal public-job chat
    // records to it (only this wallet can then decrypt them).
    if (encryptionPublicKey) body.encryptionPublicKey = encryptionPublicKey
    return this.request<RegisterKeyResult>('POST', '/api/wallet/register-key', body)
  }

  /**
   * Decrypt a sealed public-job field (a promptCipher / replyCipher envelope) with the
   * wallet's X25519 private scalar (derive it via {@link deriveEncryptionKeypair}).
   * Cleartext input is returned unchanged, so it's safe to call on any field.
   */
  decryptSealed(envelopeOrText: unknown, privateScalarB64: string): string {
    if (isEnvelope(envelopeOrText)) return openSealed(envelopeOrText, privateScalarB64)
    return envelopeOrText == null ? '' : String(envelopeOrText)
  }

  /**
   * Register a keypair generated by `generateKeyPair()` (address is pre-derived).
   * Also derives and registers the wallet's X25519 encryption key so it can receive
   * sealed public-job chat records.
   */
  async registerKeyPair(keyPair: KeyPair, rotationProof?: string): Promise<RegisterKeyResult> {
    const proof = await createSigningProof(keyPair.address, keyPair.signingPrivateKey)
    const { publicKeyB64 } = deriveEncryptionKeypair(keyPair.signingPrivateKey)
    return this.registerSigningKey(
      keyPair.address,
